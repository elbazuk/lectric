...
class filemanager
{
	...
	*/
	...
	/* HTML  function */
		...
		*/
			public function getFAIconFile(string $file = ''): string
			{
				...
				switch($ext){
					case 'aac': return 'fa-file-audio-o'; break;
					case 'mp3': return 'fa-file-audio-o'; break;
					case 'wav': return 'fa-file-audio-o'; break;
					case 'flac': return 'fa-file-audio-o'; break;
					case 'mov': return 'fa-file-video-o'; break;
					case 'wmv': return 'fa-file-video-o'; break;
					case 'mp4': return 'fa-file-video-o'; break;
					case 'avi': return 'fa-file-video-o'; break;
					
					case 'xls': return 'fa-file-excel-o'; break;
					case 'xlsx': return 'fa-file-excel-o'; break;
					case 'doc': return 'fa-file-word-o'; break;
					case 'docx': return 'fa-file-word-o'; break;
					case 'pdf': return 'fa-file-pdf-o'; break;
					case 'odt': return 'fa-file-word-o'; break;
					case 'ods': return 'fa-file-excel-o'; break;
					case 'odp': return 'fa-file-powerpoint-o'; break;
					case 'ppt': return 'fa-file-powerpoint-o'; break;
					case 'pptx': return 'fa-file-powerpoint-o'; break;
					
					case 'jpg': return 'fa-file-image-o'; break;
					case 'jpeg': return 'fa-file-image-o'; break;
					case 'png': return 'fa-file-image-o'; break;
					case 'gif': return 'fa-file-image-o'; break;
					case 'bmp': return 'fa-file-image-o'; break;
					
					case 'xml': return 'fa-file-code-o'; break;
					case 'sss': return 'fa-file-code-o'; break;
					
					case 'zip': return 'fa-file-archive-o'; break;
					case 'rar': return 'fa-file-archive-o'; break;
					case '7z': return 'fa-file-archive-o'; break;
					
					case 'csv': return 'fa-file-text-o'; break;
					case 'css': return 'fa-file-text-o'; break;
					case 'sql': return 'fa-file-text-o'; break;
					case 'sav': return 'fa-file-text-o'; break;
					case 'txt': return 'fa-file-text-o'; break;
					case 'json': return 'fa-file-text-o'; break;
	...
	/* UTITLITY  function */
        ...
        /**
         * Get the files and folders within a directory
         * 
        ...
        /**
         * Make human readable filesize string
         * 
         * @param int $bytes 
         * @param int $decimals  
         * 
         * @return <type>
         */
			public function fileSizeHuman(int $bytes, int $decimals = 2): ?string
			{
			  $sz = 'BKMGTP';
        ...
        /**
         * Make a new directory within a folder, with a clean name
         * 
         ...
         * @return bool
         */
			public function makeNewDirectory(string $folderName, string $directory): bool
			{
		...
		public function uploadFile(): void
		{
			...
			//any attacks or suspicious params?
				if (!isset($_FILES['fm_upload_file']['error']) || is_array($_FILES['fm_upload_file']['error'])){
					echo '{"error":true, "info": "Invalid Parameters."}';
					exit;
				}
			
			//any litteral errors
				switch ($_FILES['fm_upload_file']['error']) {
					case UPLOAD_ERR_OK:
						break;
					case UPLOAD_ERR_NO_FILE:
						$message =  'No file sent.';
						die('{"error":true, "info": "'.$message.'."}');
			...
			//does it exceed a certain file size?
				if(!defined('MAX_UPLOAD_FILE_SIZE')){
					define('MAX_UPLOAD_FILE_SIZE',104857600 ); //100mb 
				}
				if ($_FILES['fm_upload_file']['size'] > MAX_UPLOAD_FILE_SIZE) {
					$message =  'Exceeded filesize limit.';
					die('{"error":true, "info": "'.$message.'."}');
				}
			
			//get chunk numbers, or if none sent then 0
			...
			//check ext first...
				$extBits = explode('.',$fileName);
				$fExt = end($extBits);
				$allowed = false;
				foreach($this->_allowed_file_types as $ext => $mime){
					if($ext === $fExt){
						$allowed = true;
						break;
					}
				...
				if($allowed === false){
					die('{"error":true, "info": "File type not allowed. '.htmlentities($fExt).'"}');
				}
			
			//chunk filename
				$chunkName = $fileName.'.part_'.$chunk;
				
			//chunk destinations
				$chunkFileDest = DOC_ROOT.'/temp/'.$chunkName;
				$tempFileDest = DOC_ROOT.'/temp/'.$fileName;
			...
			if(move_uploaded_file($_FILES['fm_upload_file']['tmp_name'], $chunkFileDest) === true){
				...
				//return if chunk successful, but not the last chunk.
				if($chunks > 0 && ($chunk < ($chunks-1))){
					
					die('{"error":false, "info":"Chunk Upload Successful.", "chunk": "'.$chunkName.'", "chunks":'.$chunks.'}');
					
				//if it's the last chunk (or only chunk)
				} else if($chunks == 0 || $chunk == ($chunks - 1)) {
					
					//open file to be written to
					$finalFileHandle = fopen($finalFileDest, 'wb');
					
					if($finalFileHandle !== false){
						
						//loop through chunks and make file.
						$i = 0;
						while($i <= ($chunks-1)){
							
							// Read binary input stream and append it to temp file
							$chunkFileHandle = fopen($tempFileDest.'.part_'.$i, "rb");
							
							if ($chunkFileHandle !== false) {
								
								while ($buff = fread($chunkFileHandle, 4096)){
									fwrite($finalFileHandle, $buff);
								}
								
								unlink($tempFileDest.'.part_'.$i);
								fclose($chunkFileHandle);
								
							} else {
								die('{"error":true, "info": "Failed to add chunk to main file."}');
							}
						...
						//check the mime type thoroughly
							...
							@$finfoFileMime = $finfo->file($finalFileDest);
							if ($finfoFileMime === false){
								$message =  'Invalid file format';
								die('{"error":true, "info": "'.$message.'."}');
							} else {
								
								//check file mime type and ext together with final file...
								//$fExt set above
								$allowed = false;
								$which = '';
								foreach($this->_allowed_file_types as $ext => $mime){
									if($fExt === $ext && $finfoFileMime == $mime){
										$allowed = true;
										break;
									}
								}
								...
								if($allowed === false){
									unlink($finalFileDest);
									die('{"error":true, "info": "File ext / mime-type not allowed. '.htmlentities($fExt).' - '.htmlentities($finfoFileMime).'"}');
								}
	...
	/* JS Function */
         ...
         */
				...
				?>
					...
					<script>
						...
						function makeLoading(){
							$('.fm_content').html('<p class="text-centered end" style="padding:1em;"><i class="fa fa-cog fa-3x fa-spin fa-lg"></i></p>');
						...
						//upload a file via chunked ajax
							var spssUploader = new plupload.Uploader({
								runtimes : 'html5,html4',
								browse_button : 'fm_upload_file', // you can pass an id...
								container: document.getElementById('fm_upload_form'), // ... or DOM Element itself
								url : '/do/response/filemanager/upload/',
								chunk_size: '1mb',
								multi_selection:false,
								file_data_name:'fm_upload_file', //$_FILES['spss_file']...
								/*filters : {
									max_file_size : '100mb',
									mime_types: [
										{title : "SPSS Files", extensions : "sav"}
									]
								},*/
								init: {
									Init: function (){
										console.log('plupload ok');
									},
									PostInit: function() {
									},
									...
									BeforeChunkUpload: function(up, file, post, current, offset){
										post.directory = $('#fm_global_dir').val();
									...
									Error: function(up, err) {
										...
										if(err.code == plupload.FILE_SIZE_ERROR){
											showMessage('<i class="fa fa-ban fa-fw"></i> Oops! This file is too big to upload. Max file size 100mb.', 'red');
										} else {
											spssUploader.stop();
											showMessage('<i class="fa fa-ban fa-fw"></i> Oops! Something went wrong uploading your file:' +err.message, 'red');
		 ...
		 /**
         * Output the JS needed for tinymce
         * 
         * 
         * @return <type>
         */
			public function getTinyMCEJS(): void
			{
         ...
         */
			public function getCSS(): void
			{
				...
				?>
					...
					.fm_wrapper *,.fm_wrapper :after,.fm_wrapper :before{
					...
					.fm_top_bar input,.fm_top_bar select{
						height:30px;
						padding:5px
					}
					.fm_footer{
					...
					#fm_progress,.fm_new_folder_form,#fm_upload_form{
						position:absolute;
						height:40px;
						line-height:40px;
						width:100%;
					...
					.fm_new_folder_form,#fm_upload_form{
						background:grey;
						background:rgba(128,128,128,.8);
						color:#fff
					}
					.fm_new_folder_form table td,#fm_upload_form table td{
						padding:4px;
						vertical-align:middle;
						border:0;
						height:40px
					}
					.fm_add_folder_submit,.fm_delete_file_submit,.fm_delete_folder_submit{
						color:green
					}
					.fm_hide_elem,.fm_top_bar_slide_up{
						color:red
					}
					.fm_breadcrumb{
						float:right
					...
					.fm_contents_table td,.fm_contents_table th{
						border-bottom:1px solid #ebebeb;
						padding:8px 15px;
						font-size:1em
					}
					.fm_error_message,.fm_no_contents{
						text-align:center
					}
					.fm_content{
						width:100%;
					...
					.fm_contents_table th{
						color:#797979
					}
					.fm_contents_table td{
						text-align:left;
						height:37px
					}
					.fm_contents_table tr td:last-child,.fm_contents_table tr th:last-child{
						text-align:center
					}
					.fm_contents_table td:hover,.fm_contents_table tr:hover{
						background-color:#e0e0e0!important
					}
					.fm_contents_table i.fa-folder{
						color:#ffd44d
					}
					.fm_file_click_form{
						display:none;
					...
					.fm_delete_file,.fm_delete_folder{
						display:none;
						height:100%;
						height:37px;
						line-height:37px;
